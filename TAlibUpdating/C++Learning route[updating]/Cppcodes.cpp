...
BY : LucasCai -> @Drink_H2O
注：本源代码用于 学习与交流，比参考<菜鸟教程>对于C++相关的教学目录进行全文编排
同时也引入编者的个人思路与代码理解，对一些细节问题进行多方查证注释
本篇编写初心是尽可能以一份代码文件通关C++的基础代码编写和语言特色
为了照顾到各位同志们看不来这份资料，我专门设计了一份word文档和md文档供给各位读者阅览
随着现在科学知识的进步和发展，知识迭代地越来越快，但基础永远是基础，地基是变不了的
同志们，我们不要焦躁，学习不是快慢的事，是早晚的事，我们要慢慢来，不要着急，不要急躁，带着好奇，带着自己的疑问去学习，基础就是一切！
阅读方式：本文的阅读方式很简单，就是从头读到尾，但是跳过开头有关函数的描述代码，有疑问的地方可以先保留，或者自行查询，后续会尽可能遇到并解决大部分的问题
本教程并不完全覆盖各种知识点，读者自学期间有疑问就要立马问，广泛求知，求到底层原理，格物而致知
如果还有什么疑惑没有在文中解决，读者可以自行查找参考文献，询问AI工具，或者邮件留言:2135504482@qq.com
*/

/*
上面是一个程序文件描述，这通常是一种便于协作的代码编写规范，建议各位同志们都要加上，在文件多的时候能分辨和区分他们的作用
...
{
    public:
    friend void swept(freswep swep);
    friend void printswept(freswep swep);
    freswep(int a, int b){
        xa = a;
        yb = b;
        cout << "交换对象已创建" << endl;
...
//友元函数
void swept(freswep swep){
    int temp = swep.xa;
    swep.xa = swep.yb;
    swep.yb = temp;
}

void printswept(freswep swep){
    
    cout << "当前顺序 ： " << swep.xa << " and " << swep.yb << endl;
...
class Thisclass
{
    ...
    void printvalue(){
        cout << "value : " << this->value << "and" << this->val << endl;
    }
...
//实际上，this通常是隐式的
//cout那一栏，你可以不写任何this，甚至整个Thisclass都可以不写this
//也就是说你写value = value也是可以的，因为编译器可以识别左值右值的区别，它会进行优化
//但同名冲突建议写上，避免其他错误
...
//为什么这里不打缩进了？原因就在于，这里打的缩进会被认为是空格并被打印出来
    ...
    */
    ...
    /*  ---友元函数---  */
    /*
    这个非常简单，friend看作一个类型或者权限(实际我们不这么叫)
    只能在类内进行friend标记
    我们称被标记函数或类是当前类的友元 
    */
    freswep swep1(1,2);
    ...
    /*  ---内联函数---  */
    /*
    在理解内联函数之前，你需要知道函数的调用流程
    正常情况下我们的编译器识别到函数使用语句后会调用函数
    它会获取当前的传入参数，然后跑到上面或者下面找到这个函数
    接着把值传给它，一通函数内计算后return出一个值
    编译器会把值运回来给到我们的语句中处理
    这个过程非常耗时间，为了解决效率的问题
    我们提出内联函数
    内联函数的理念就是与其来回跑让你干活，不如我直接把你的干活流程复制了
    然后放在这里直接算
    定义呢也很简单，就是在一个函数前面加inline
    注意：inline是一个请求，而非命令
    编译器会根据实际情况，如变量是否冲突，代码是否过多过复杂，是否使用了复杂循环等等
    来选择是否采纳内联
    编译器很聪明，如果你平时定义了一个非常简单的函数，代码可能就一行，两行，总之很简单
    即便你没有发出inline需求，编译器也可能会采纳inline
    所以有些教材里说inline只能处理不超过一行的代码本质是不对的
    */
    int aerf = 12;
    int frea = 21;
    cout << "相比之下 " << big(aerf,frea) << " 更大" << endl;

    //this指针
    /*
    this是C++的一个特殊指针，它指向当前对象的实例
    用到访问运算符->
    何为实例？
    我们先前从未介绍过这个概念，但它已经存在了
    一个最简单的例子就是创建类的对象
    没错，我们定义了一个类，在创建它的时候，这个类就被实例化了
    用一个比喻来讲，就是，类是图纸或者一个模板
    而我们创建对象的时候，就是在用图纸模板创造了一个它的实在事务
    既然如此，我们类比到数据存储方式上
    类本身不被存储，它是说明书，你不用记住它，只需要读懂它
    而类的对象需要被存储，因为他是真正通过这个说明书创造出来的
    所以，this指向对象的实例知道是什么了吗？
    就是对象内部具体存在的变量等等
    回到头部，可以看到我们对this的使用
    */
    ...
    //指向类的指针
    /*
    一个指向C++类的指针和指向结构的指针一样，都是为了访问其成员
    因此需要用到访问运算符->
    我们拿先前定义的Line类来作例子
    class Line 
    {
    ...
    */
    Line underline;
    underline.setLength(3.1);
    Line *p_uline = & underline;
    cout << "Line length  : " << p_uline->glen << endl;
    //类的指针也无法直接访问private和protected中的东西
    p_uline->getLength();
    
    //我们知道如果要动态分配内存，创建一个分配指针是不错的选择
    //结合到我们的类，我们可以创建分配指针，用动态分配内存的方式创建对象
    //这样还能最终把这个新建的类删掉
    //不过，通过指针分配的方式依然只能访问public权限内容
    Line *dep_unline = new Line;
    dep_unline->glen = 42;
    dep_unline->getLength();
    delete dep_unline;

    //通过这种方式，我们可以通过指针传参
    //也就是类内的函数指针用法，请读者自行求解哦
